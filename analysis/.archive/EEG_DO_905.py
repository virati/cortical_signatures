...
def extract_raw_mat():
    ...
    pt_dir = ""
    file = "DBS905_TurnOn_Day1_onTARGET_20150928_015403.mat"
    ...
    T, F, SG = sig.spectrogram(
        sg_sig, nfft=2**10, window="blackmanharris", nperseg=512, noverlap=0, fs=1000
    )
    plt.figure()
    plt.pcolormesh(F, T, np.log10(SG))

    t_bounds = {"Pre_STIM": (600, 640), "BL_STIM": (660, 830)}
    ...
    # Save DataStructure
    sio.savemat("/tmp/DBS905_chirp", signal)
...
for condit in ["OnTarget"]:
    ...
    test_dec = sig.detrend(test_dec, axis=1, type="linear")
    sos = sig.butter(10, (5, 20), btype="bandpass", fs=1000 / 10, output="sos")
    for ii in range(test_dec.shape[0]):
        test_dec[ii, :] = sig.sosfilt(sos, test_dec[ii, :])
    plt.plot(test_dec.T)
    ...
    for elabel, ebos in snippets.items():
        ...
        for ch in range(257):
            sig_filt = sig.decimate(
                data[ch, ebos[0] : ebos[1]], ds_fact, zero_phase=True
            )
    ...
    if len(ch) == 1:
        ...
        sel_sig = sig.decimate(
            data[ch[0]][:] - data[ch[1]][:], ds_fact, zero_phase=True
        )
    ...
    F, T, SG = sig.spectrogram(
        ...
        nperseg=512,
        noverlap=500,
        window=sig.get_window("blackmanharris", 512),
        fs=fs / ds_fact,
    )
