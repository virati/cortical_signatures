...
if Phase == "TurnOn":
    ...
    # 901
    Ephys["901"]["OnTarget"]["Filename"] = (
        "/run/media/virati/Samsung USB/MDD_Data/BR/901/Session_2014_11_14_Friday/DBS901_2014_11_14_16_46_35__MR_0.txt"
    )
    Ephys["901"]["OffTarget"]["Filename"] = (
        "/run/media/virati/Samsung USB/MDD_Data/BR/901/Session_2014_11_14_Friday/DBS901_2014_11_14_17_34_35__MR_0.txt"
    )
...
for pp, pt in enumerate(pt_list):
    ...
    for cc, condit in enumerate(["OffTarget"]):
        ...
        for seg in do_segs:
            ...
            start_idx = min(
                range(SGs[pt][condit]["Left"]["T"].shape[0]),
                key=lambda i: abs(
                    SGs[pt][condit]["Left"]["T"][i]
                    - Ephys[pt][condit]["segments"][seg][0]
                ),
            )
            end_idx = min(
                range(SGs[pt][condit]["Left"]["T"].shape[0]),
                key=lambda i: abs(
                    SGs[pt][condit]["Left"]["T"][i]
                    - Ephys[pt][condit]["segments"][seg][1]
                ),
            )
            ...
            plt.plot(
                SGs[pt][condit]["Left"]["F"],
                10 * np.log10(SGs[pt][condit]["Left"]["SG"][:, middle_idx]),
            )
        plt.legend(do_segs)
# %%
if 1:
    for pp, pt in enumerate(pt_list):
        ...
        for cc, condit in enumerate(["OnTarget", "OffTarget"]):
            ...
            plt.title(condit)
            plt.pcolormesh(
                SGs[pt][condit]["Left"]["T"],
                SGs[pt][condit]["Left"]["F"],
                10 * np.log10(SGs[pt][condit]["Left"]["SG"]),
                rasterized=True,
            )
            plt.subplot(2, 2, 2 * cc + 2)
            plt.title(condit)
            plt.pcolormesh(
                SGs[pt][condit]["Right"]["T"],
                SGs[pt][condit]["Right"]["F"],
                10 * np.log10(SGs[pt][condit]["Right"]["SG"]),
                rasterized=True,
            )
...
condit = "OffTarget"
plt.pcolormesh(
    SGs[pt][condit][side]["T"],
    SGs[pt][condit][side]["F"],
    10 * np.log10(SGs[pt][condit][side]["SG"]),
    rasterized=True,
)
plt.colorbar()
# %%
...
def Plot3D(pt, side, condit):
    ...
    ax.plot_surface(
        F[F_idxs, None],
        T[None, T_idxs],
        10 * np.log10(SGs[pt][condit][side]["SG"][np.ix_(F_idxs, T_idxs)]),
        cmap=cm.coolwarm,
    )
    plt.show()
...
def grid_dyn(pt, condit):
    ...
    for ii in range(xg.shape[0] - 1):
        for jj in range(yg.shape[0] - 1):
            pts_in_cell = np.where(
                np.logical_and(
                    np.logical_and(state[0, :] < xg[ii + 1], state[0, :] > xg[ii]),
                    np.logical_and(state[1, :] < yg[jj + 1], state[1, :] > yg[jj]),
                )
            )
            if len(pts_in_cell[0]) != 0:
                try:
                    changes = np.median(sd[:, pts_in_cell].squeeze(), axis=1)
                except:
                    ipdb.set_trace()
                # pdb.set_trace()
