...
class proc_dEEG:
    def __init__(
        ...
        self.Feat_diff = {
            pt: {condit: [] for condit in self.condits} for pt in self.pts
        }
        ...
        for pt in pts:
            ...
            for condit in self.condits:
                ...
                for epoch in keys_oi[condit]:
                    ...
                    }  # transpose is done to make it segxtime
    ...
    def plot_median_response(
        self, pt="POOL", band="Alpha", condit="OnT", use_maya=False
    ):
    ...
    def OnT_dr(self, pt="POOL", data_source=[]):
        if data_source == []:
            ...
            med_response = self.median_bootstrap_response(pt=pt)["mean"][
                "OnT"
            ]  # If you want the bootstrap response
        ...
        all_stack = np.concatenate(
            [
                np.concatenate(
                    [
                        np.concatenate(
                            [ref_stack[condit][epoch][pt] for pt in self.pts], axis=0
                        )
                        for epoch in ["OF", "ON"]
                    ],
                    axis=0,
        ...
        avg_change = {
            pt: {
                ...
                * (
                ...
                for pt, condit in itertools.product(self.pts, self.condits)
            }
        }
    ...
    def DEPRgen_OSC_stack(self, stack_type="all"):
        ...
        for condit in self.condits:
            ...
            big_stack[condit] = {
                remap[epoch]: {pt: self.osc_dict[pt][condit][epoch] for pt in self.pts}
                for epoch in keys_oi[condit]
            }
