...
for pp, pt in enumerate(all_pts):
    for cc, condit in enumerate(["OnT", "OffT"]):
        for ss, side in enumerate(["L", "R"]):
            ...
            data[pp][condit][side] = image.smooth_img(
        ...
        combined[pt][condit] = image.math_img(
            "img1+img2", img1=data[pp][condit]["L"], img2=data[pp][condit]["R"]
        )
...
condit_avg = nestdict()
for cc, condit in enumerate(["OnT", "OffT"]):
    condit_avg[condit] = image.math_img(
        "np.mean(np.array([img4,img5,img6]),axis=0)",
        img1=combined["901"][condit],
        img2=combined["903"][condit],
        img3=combined["905"][condit],
        img4=combined["906"][condit],
        img5=combined["907"][condit],
        img6=combined["908"][condit],
    )
    ...
    plotting.plot_glass_brain(
        condit_avg[condit],
        black_bg=True,
        title=condit + " average tractography",
        vmin=0,
        vmax=2,
    )
...
diff_map = image.math_img(
    "np.abs(img1-img2)", img1=condit_avg["OnT"], img2=condit_avg["OffT"]
)
plotting.plot_glass_brain(
    diff_map, black_bg=True, title="Diff Tractography", vmin=0, vmax=1, threshold=0.5
)
